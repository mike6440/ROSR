...
const unsigned CCWSTOPMILLISECS = 100;
const unsigned CWSTOPMILLISECS = 100;

struct eeprom {
    byte id, Nbb, Nsky, Nocean, ShutterFlag, CalFlag;
    unsigned int cwstopmsec, ccwstopmsec;
...
void    Action(char *cmd)
{
    ...
    if(cmd[0] == '?'){
        ...
        Serial.println("Einnn   -- Shtr close(cw) delay     EInnn   -- Shtr open(ccw) delay");
        Serial.println("");
...
//             }
                ...
                ix = atoi(cmd+2);
                if(ix == 0){Serial.println("RUN");}
                else if(ix == 1){Serial.println("CALIBRATION");}
                else {Serial.println("UNDEFINED, NO CHANGE");}
                ee.CalFlag=ix;
                eok=1;
            }
            else if( cmd[1]=='r' ){
            ...
            // 'i'
                ...
                ix = atoi(cmd+2);
                Serial.print("SET Shutter delay close(CW) delay = "); 
                Serial.print(ix,DEC);
                Serial.println(" msec");
                ee.cwstopmsec = (unsigned int)ix;
                eok=1;
            }
            // 'I'
            else if( cmd[1]==73 ){
                ix = atoi(cmd+2);
                Serial.print("SET Shutter delay open(CCW) delay = "); 
                Serial.print(ix,DEC);
                Serial.println(" msec");
                ee.ccwstopmsec = (unsigned int)ix;
                eok=1;
            }
            else if( cmd[1]=='U' ){
                ix = atoi(cmd+2);
...
void EepromDefault(){
    ...
    ee.ebb = default_ebb;
    ee.cwstopmsec = CWSTOPMILLISECS;
    ee.ccwstopmsec = CCWSTOPMILLISECS;
    EepromStore();
    EepromRead();
    EepromPrint();
}
...
void EepromPrint()
{
    ...
    Serial.println(" deg");
//     Serial.print("  i Shutter close(CW) delay  = ");  
//     Serial.print(ee.cwstopmsec,DEC);  
//     Serial.println(" msec");
//     Serial.print("  I Shutter open(CCW) delay  = ");  
//     Serial.print(ee.ccwstopmsec,DEC);  
//     Serial.println(" msec");
