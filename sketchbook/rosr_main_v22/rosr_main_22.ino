...
// These constants are set for ROSR1 because the tilt sensor was mounted
//  backwards. For other sn's we expect the tilt sensor will be pointed 
//  properly. Then P1=R1=1;
...
const unsigned CWSTOPMILLISECS = 300;
...
const unsigned long default_rainsecs = 300;
...
void EepromPrint()
{
    ...
    Serial.println(ee.ShutterFlag,2);  
...
float PointScanDrum(float requestpos) {
    ...
    //Serial.print("encoderref = "); Serial.println(ee.encoderref,1);
...
void ShutterOpen(void) {
    ...
    // CHECK STATE  
    if(digitalRead(DHe1)==0 && digitalRead(DHe2)==1) {
    	//Serial.println("Shutter is already open.");
        ShutterState=OPEN;
        return;
    }
    ...
    while( !Serial.available() ){
        ...
        if( digitalRead(DHe1)==0 ) {
            Serial.println("SHUTTER OPEN, HE1");
            	// ALLOW EXTRA TIME TO CLOSE
            t1=millis();
            while( (unsigned)(millis() - t1) < CCWSTOPMILLISECS ){
            	if(abs(GetMotorCurrent()) > CCWSHUTTERCURRENTTHRESHOLD) {
            		Serial.println("He1 current Limit.");
            		break;
            	}
            }
